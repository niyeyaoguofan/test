...
export function computeTerritory(board: Board, tiles: (Tile | null)[][]): { controlled: boolean[][]; territoryCount: number; continuityBonus: number } {
  ...
  for (let y = 0; y < board.height; y += 1) {
    for (let x = 0; x < board.width; x += 1) {
      ...
      const radius = tile.subtype === 'beacon' ? 2 : 1;
  ...
  return { controlled, territoryCount, continuityBonus };
}
